    ...
    def forward_train(self,
        ...
        if self.with_occ_head:
            if outs_motion['track_query'].shape[1] == 0:
                # TODO: rm hard code
                outs_motion['track_query'] = torch.zeros((1, 1, 128)).to(bev_embed) # 원본: 256 / 수정: 128
                outs_motion['track_query_pos'] = torch.zeros((1,1, 128)).to(bev_embed) # 원본: 256 / 수정: 128
                outs_motion['traj_query'] = torch.zeros((3, 1, 1, 6, 128)).to(bev_embed) # 원본: 256 / 수정: 128
