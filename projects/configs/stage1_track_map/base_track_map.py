...
bev_h_ = 200 #_ 초기값: 200 수정log:50 // Bird's Eye View(BEV) 이미지의 높이
bev_w_ = 200 #_ 초기값: 200 수정log:50 // Bird's Eye View(BEV) 이미지의 너비
...
model = dict(
    ...
    num_query=900, #_ 수정log:500 // DETR(Deformable DETR) 모델에서 사용하는 쿼리 수
    num_classes=10,
    pc_range=point_cloud_range,
    img_backbone=dict(
        type="ResNet",
        depth=101, #_ 초기값: 101 수정log:50 // ResNet 백본의 깊이, 레이어 수, ResNet 네트워크의 깊이
        num_stages=4,
        out_indices=(1, 2, 3),
        frozen_stages=4,
    ...
    seg_head=dict(
        ...
        pc_range=point_cloud_range,
        num_query=300, #_ 수정log: 150 // Segmentation transformer에서 사용하는 쿼리 수
        num_classes=4,
